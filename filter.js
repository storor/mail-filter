...
function cacheMatch(value, expression, useCache){
  if(useCache){
...
function apply(message, rule, useCache){
  let actual = 0
  let expected = 0;
  if (rule.from) {
    expected++;
    if (match(message.from, rule.from, useCache)) {
      actual++;
    }
  }
  if (rule.to) {
    expected++;
    if (match(message.to, rule.to, useCache)) {
      actual++;
    }
  }
  if (expected == actual) {
...
function filter(messages, rules, useCache){
  var result = {},
      messagesKeys;
  ...
  for(let rule of rules){
    ...
    for(let key in messages){
      let applied = apply(messages[key], parsedRule, useCache);
      if(!result[key]){
        result[key] = [];
      }
      if(applied){
